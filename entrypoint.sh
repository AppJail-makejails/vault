...
#
# Code based on: https://github.com/hashicorp/vault/blob/54f1daa659d823676ec0f01ba9e1386a2c0bb0f4/.release/docker/docker-entrypoint.sh
...
get_addr() {
    ...
    ifconfig "$if_name" | gawk -v uri="$uri_template" '/\s*inet\s/ { \
      ip=gensub(/(.+)\/.+/, "\\1", "g", $2); \
...
if [ -n "$VAULT_LOCAL_CONFIG" ]; then
    ...
    chown noroot:noroot "$VAULT_CONFIG_DIR/local.json"
fi
